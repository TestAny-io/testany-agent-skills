...
export function installLauncher({destination=path.join(os.homedir(),'Applications/TeamDesk.app'),desktopLink=true,runtimeConfig={node:process.execPath,codexHome:path.resolve(process.env.CODEX_HOME||path.join(os.homedir(),'.codex'))}}={}) {
  ...
  try {
    ...
    const info={CFBundleIdentifier:identifier,CFBundleName:'TeamDesk',CFBundleDisplayName:'TeamDesk',CFBundleExecutable:'TeamDesk',CFBundlePackageType:'APPL',CFBundleShortVersionString:pluginVersion,CFBundleVersion:pluginVersion,CFBundleIconFile:'TeamDesk.icns',LSUIElement:true,LSMinimumSystemVersion:'13.0',NSHighResolutionCapable:true};
    execFileSync('/usr/bin/plutil',['-convert','xml1','-o',path.join(app,'Contents/Info.plist'),'-'],{input:JSON.stringify(info)});
    execFileSync('/usr/bin/xcrun',['swiftc','-swift-version','5','-O',path.join(assets,'LauncherCore.swift'),path.join(assets,'TeamDeskLauncher.swift'),'-o',path.join(macos,'TeamDesk')],{stdio:'inherit',timeout:180000});
    execFileSync('/usr/bin/xcrun',['swiftc','-swift-version','5','-parse-as-library','-O',path.join(assets,'LaunchCodex.swift'),'-o',path.join(resources,'launch-codex')],{stdio:'inherit',timeout:180000});
    ...
    execFileSync('/usr/bin/xcrun',['swiftc','-swift-version','5',path.join(assets,'Icon.swift'),'-o',render],{stdio:'inherit',timeout:180000});
    ...
    return {installed:true,version:pluginVersion,app:destination,desktopLink:link||null};
