...
export function InstallDialog({ kind, data, initialPlugin, initialMarket = false, busy, action, onClose, onMarket }: {
  ...
  const [selection, setSelection] = useState<Plugin | null>(initialPlugin || null);
  const [error, setError] = useState(""); const review = preview || selection;
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const result = await action(selection ? { action: "plugin.install", id: selection.id } : preview
        ? { action: plugin ? "plugin.installSource" : "skill.install", previewId: preview.id }
      ...
      if (result?.pluginPreview || result?.preview) setPreview(result.pluginPreview || result.preview!);
      else if (result) onClose();
    } catch (failure) { setError((failure as Error).message); }
  }
  return <Modal wide={sourceType === "market" && !review} title={t(review ? plugin ? "确认安装插件" : "确认安装技能" : plugin ? "安装插件" : "安装技能")} eyebrow={plugin ? "INSTALL A PLUGIN" : "INSTALL A SKILL"} onClose={() => { if (!busy) onClose(); }}>
    <form onSubmit={event => void submit(event)}>
      <div className="modal-body">
        <InstallSteps preview={!!review} />
        {review ? <>
          <div className="install-preview"><span className="skill-icon tone-1">{plugin ? <Blocks size={23} /> : <Sparkles size={23} />}</span><h3>{review.name}</h3><p>{review.description || t("未提供描述")}</p>
            {"version" in review && <span className="badge badge-neutral">{review.version || t("版本未提供")}</span>}
            {preview && <span className="badge badge-neutral">{t("{v0} 个文件", { v0: preview.files })} · {(preview.bytes / 1024).toFixed(1)} KB</span>}
          </div>
          ...
          {direct && <div className="install-components"><strong>{t("附带技能（{v0}）", { v0: direct.skills.length })}</strong><p>{direct.skills.join(" · ") || t("没有附带技能")}</p>
          ...
          {sourceType === "market" ? <div className="market-install">
            ...
            <div className="install-catalog">{listed.slice(0, limit).map(item => <button type="button" className="install-catalog-row" key={item.id} disabled={!!busy || item.installed || !item.canInstall} onClick={() => { setSelection(item); setError(""); }}>
