...
此目录是可复现的 app 源码，随独立 `skilldock` 插件分发，仅包含一个 `skill-manager` 入口。当前版本 0.5.0 统一 Skills / Plugins 的布局与安装流程，支持从本地目录或 Git 目录链接直接安装单个插件，并提供已有项目下拉选择与文件夹选择器。保留插件外部同步后的基线恢复、macOS 独立后台定时更新、更新进度、逐行 diff、本机标签及筛选、私仓认证、CLI 回退和同名技能按路径选择移除。
