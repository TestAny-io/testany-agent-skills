...
此目录是可复现的 app 源码，随独立 `skilldock` 插件分发，仅包含一个 `skill-manager` 入口。当前版本 0.6.0 提供安装前的插件技能清单和逐项启用选择，修复单技能开关会影响整个插件的问题；Skills / Plugins 沿用统一的布局与安装流程，支持从本地目录或 Git 目录链接直接安装单个插件，并提供已有项目下拉选择与文件夹选择器。保留插件外部同步后的基线恢复、macOS 独立后台定时更新、更新进度、逐行 diff、本机标签及筛选、私仓认证、CLI 回退和同名技能按路径选择移除。
...
插件内技能选择：三种安装来源都在确认前读取实际技能，显示名称、用途、相对目录，支持搜索、全选、全不选及按路径勾选。同名技能保持分开；读取失败时显示错误，不会冒充“没有技能”。**插件依然整包安装，勾选控制哪些技能启用**；未勾选技能保留文件但禁用，MCP、Hooks 等其他组件仍随插件安装。确认前的预览和取消不安装插件。
