...
export async function runBackground(context, options = {}) {
  ...
  let status = { ...previous, version, lastWakeAt: stamp(), pid: process.pid };
  ...
  if (!schedule?.enabled || disabled) {
    await report({ finishedAt: stamp(), outcome: 'disabled', error: undefined });
  ...
  if ((!due && !previous.runtimePending && !installationChanged) || previous.retryAt && Date.parse(previous.retryAt) > clock()) {
    await report({ finishedAt: stamp() }); return status;
  }
  let release;
  ...
  try {
    ...
    if (!source) {
      ...
      await report({ outcome: 'uninstalled', finishedAt: stamp(), error: undefined });
      return status;
    }
    ...
    const failed = latest.runs?.[0]?.items.find(item => ['error', 'skipped'].includes(item.status));
    await report({ outcome: next === null ? 'uninstalled' : progress?.status || 'success', finishedAt: stamp(),
      error: failed?.message, retryAt: undefined, failureCount: 0, runtimePending: false, installationStamp });
  } catch (error) {
    const failureCount = (previous.failureCount || 0) + 1;
    await report({ outcome: 'error', finishedAt: stamp(), error: redact(error.message), failureCount,
