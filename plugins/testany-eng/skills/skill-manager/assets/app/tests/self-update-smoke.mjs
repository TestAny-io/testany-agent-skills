...
for (const directory of [codexHome, state, market, project]) await fs.mkdir(directory);
...
try {
  ...
  result = { passed: true, realCodexCli: cli, root, sourceType: gitSource ? 'git' : 'local', bootstrap, execution: record.execution, node: process.versions.node, oldPid: old.pid, newPid: after.pid,
    oldDigest: before.sourceDigest, newDigest: after.sourceDigest, planPreserved: true, nextScheduledRunPassed: true, historyPreserved: true, installedVersion: '0.2.0' };
} catch (error) {
