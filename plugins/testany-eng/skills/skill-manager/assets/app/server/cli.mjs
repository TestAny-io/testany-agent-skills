...
export async function checkoutGit(source, ref, destination, options = {}) {
  ...
  const resolved = (await run(['-C', destination, 'rev-parse', '--verify', `${ref || 'HEAD'}^{commit}`])).stdout.trim();
...
export class CodexAdapter {
  ...
  async probe() {
    ...
    this.probePromise = (async () => {
      const candidates = this.explicit ? [this.explicit] : [path.join(this.codexHome, 'plugins/.plugin-appserver/codex'), 'codex'];
      const failures = [];
      for (const binary of candidates) {
        try {
          const version = (await runProcess(binary, ['--version'], { timeout: 5000, env: { ...process.env, CODEX_HOME: this.codexHome } })).stdout.trim();
          const help = (await runProcess(binary, ['plugin', '--help'], { timeout: 5000, env: { ...process.env, CODEX_HOME: this.codexHome } })).stdout;
          if (!/marketplace/.test(help) || !/\blist\b/.test(help)) throw new Error('CLI 不支持 plugin 管理。');
          this.info = { available: true, version: version.slice(0, 200), path: binary }; return this.info;
        } catch (e) { failures.push(redact(e.message)); }
      }
      this.info = { available: false, error: `Codex CLI 不可用。可设置 SKILLDOCK_CODEX_BIN 后重启。${failures.join('；')}` }; return this.info;
    })();
