      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          <div className="private-note">
            <ShieldCheck size={19} />
            <div>
              <strong>{t("留在你的电脑上")}</strong>
              <p>{t("本地运行，直接管理。")}</p>
            </div>
          </div>
          <div className="sidebar-version">
            <span className="tiny-dot" />
            SkillDock <span>v0.1 · {t("预览版")}</span>
          </div>
        </div>
      </aside>
            ...
            <>
              ...
              <label className="field">
                ...
                <input
                  ...
                  placeholder={
                    sourceType === "local"
                      ? "/Users/you/skills/my-skill"
                      : "https://github.com/owner/repository.git"
                  }
                />
                <small>
                  {sourceType === "local"
                    ? t("填写包含 SKILL.md 的目录，或通过下方子路径指定。")
                    : t("支持受允许的 Git 来源。请使用不含账号密码的地址。")}
                </small>
              </label>
  ...
  return (
    ...
    >
      ...
      <button
        className="text-button"
        onClick={
          skill.pluginId && !unknown && !duplicate ? onPlugin : onDetails
        }
      >
        {skill.pluginId && !unknown && !duplicate
          ? t("查看所属插件")
          : t("查看路径与状态证据")}
