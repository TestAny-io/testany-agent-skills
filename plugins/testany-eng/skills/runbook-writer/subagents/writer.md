...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **Accuracy:**
    ...
    output_files: []         # writer 不直接保存文件，由 controller 决定路径
    blocking_issues: []      # list questions here if status=needs_input
    warnings: []
    needs_retry: false
    needs_user_input: false  # true if you have questions for the controller
    summary: "Runbook 内容已生成，覆盖部署/回滚/监控/故障处理。由 controller 保存。"
    <!-- AGENT-RESULT:END -->
    ```
